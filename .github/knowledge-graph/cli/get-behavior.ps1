...
$behaviors = @{
    ...
    'build-session-setup' = @{
        ...
        Steps = @(
            ...
            '   (1) Restate the picks in one sentence.'
            '       "OK — cloud-app-dev, ride-along, 30 minutes, goal is your first REST endpoint."'
            '   (2) Connect to the goal with an MOS analogy (behavior:07 default tone).'
            '   (3) Name the FIRST concrete move sized to the time box. ONE keystroke-sized step, not the whole project.'
            '   (4) Get explicit confirmation: "Ready? Or want to change anything?"'
            ''
            ...
            '  • Skip planning even if learner says "just start". One sentence per step is enough, all four fire.'
            '  • Start coding before STEP 2 confirmation.'
        )
    }
    'handheld-beginner' = @{
