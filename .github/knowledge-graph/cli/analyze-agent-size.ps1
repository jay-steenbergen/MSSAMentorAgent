...
if ($ShowRecommendations) {
    ...
    if ($behaviors.Count -gt 5) {
        ...
        Write-Host "   Recommendation: Create cli/get-behavior.ps1"
        Write-Host "   Savings: ~30-40 lines per behavior = ~$($behaviors.Count * 35) lines"
        Write-Host "   Usage: pwsh cli/get-behavior.ps1 'identify-learner'"
    }
    ...
    if ($methods) {
        ...
        Write-Host "   Recommendation: Create cli/enforce-method.ps1"
    ...
    if ($tracks) {
        ...
        Write-Host "   Recommendation: Create cli/enforce-track.ps1"
    ...
    if ($sessionProtocols.Count -gt 0) {
        ...
        Write-Host "   Recommendation: Create cli/session-protocol.ps1"
